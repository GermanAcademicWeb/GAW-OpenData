...
def plotMimeDist(pldsurt,PLDwiseDict):
    ...
    pldsurt = 'de,uol'
    pldsurt = 'de,uni-oldenburg'
    mimeOfInterest = ['text/html','application/pdf','image/jpeg']
    ...
    for key in MimeKeys:
        ...
        try:
            ...
            datesToAdd = set(dateNames).difference(set(stats.keys()))
            for date in datesToAdd:
                d={}
                for mime in mimeOfInterest:
                    d.update()
                stats.update({date: {key: {mimeOfInterest: 0}}})
            mimeInDate = set(stats[dateNames[0]][key].keys())
        ...
        if mimeOfInterest:
            relevantMime = set(mimeOfInterest)
        else:
...
plotMimeDist('de,uol',PLDwiseDict)